...
def write_selection_preds_wow(dataset_walker, output_file, data_infos, sorted_pred_ids, inference_time, topk=5):
    # Flatten the data_infos
    data_infos = [
        {
            "dialog_id": info["dialog_ids"][i],
    ...
    for info, sorted_pred_id, elapsed_time in zip(data_infos, sorted_pred_ids, inference_time):
        dialog_id = info["dialog_id"]

        snippets = []
        for pred_id in sorted_pred_id[:topk]: 
            snippets.append(labels[dialog_id]['knowledge_candidate'][pred_id])
        
        new_label = {"target": True, "knowledge": snippets, 'selection_time_cost': elapsed_time}
        ...
        if label is None:
            ...
            label = label.copy()
            label.pop('knowledge_candidate')
            if 'hard_candidate' in label:
                label.pop("hard_candidate")
...
def write_selection_preds(dataset_walker, output_file, data_infos, sorted_pred_ids, inference_time, topk=5):
    # Flatten the data_infos
    data_infos = [
        {
            "dialog_id": info["dialog_ids"][i],
...
def write_generation_preds(dataset_walker, output_file, dialog_ids, responses, all_samples_result, inference_time):
    labels = [label for log, label in dataset_walker]
    new_labels = [{"target": False}] * len(dataset_walker)
    # Update the dialogs with detection result
    for dialog_id, response, score, time in zip(dialog_ids, responses, all_samples_result, inference_time):
        label = labels[dialog_id]
        new_label = {"target": True, "response": response, "score": score, "generate_time_cost": time}
        if label is None:
            label = new_label
        else:
            label = label.copy()
